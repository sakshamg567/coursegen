...
export const generateLesson = async ({ event }: { event: any }) => {
  const { lesson_id, title, objective, previous_error, is_retry } = event.data;
  console.log(`Starting lesson generation for lesson ${lesson_id}`);
  ...
  while (attempt < MAX_RETRIES) {
    attempt++;
    console.log(`Attempt ${attempt}/${MAX_RETRIES} for lesson ${lesson_id}`);
...
Please generate a corrected version that addresses this error.` +
    ...
    try {
      ...
      console.log(`Successfully completed lesson ${lesson_id}`);
  ...
  console.error(
    `Lesson ${lesson_id} generation failed after all retries:`,
    lastErr,
  );
